...
###data import from sav form
data7_20ch <- read_sav("C:/Users/HP/Documents/HENRY/STRATHMORE/project/dhs/KEKR81SV_ken2020/KEKR81FL.sav")
View(data7_20ch)
...
#Weibull: dist = "gompertz"
model_igbgan <- parfm(Surv(time_to_itnusage, trtdnet_std) ~ child_age+mal_symptoms_std,
                     cluster = "hold",
                     data = new_data4trial,
                     dist = "gompertz",   # Baseline hazard: exponential, weibull, gompertz, loglogistic, lognormal
                     frailty = "none")     # Inverse Gaussian frailty
print(summary(model_igbgan))
...
coef_matrixbgan <- model_igbgan

# Identify the relevant covariates (sm_phone_std+int_use_std+soc_media_std)
covariatesbgan <- c("child_age", "mal_symptoms_std")

# Extract estimates, standard errors, and p-values for covariates
resultsbgan <- data.frame(
  Estimate = round(coef_matrixbgan[covariatesbgan, "ESTIMATE"], 3),
  SE = round(coef_matrixbgan[covariatesbgan, "SE"], 3),
  `z-value` = round(coef_matrixbgan[covariatesbgan, "ESTIMATE"] / coef_matrixbgan[covariatesbgan, "SE"], 2),
  `p-value` = signif(coef_matrixbgan[covariatesbgan, "p-val"], 4)
)

# Extract frailty variance (theta), log-likelihood, AIC, and BIC
frailty_variancegan <- round(coef_matrixbgan["theta", "ESTIMATE"], 3)
log_likelihoodgan <- round(attr(model_igbgan, "loglik"), 2)
AIC_val <- round(-2 * log_likelihoodgan + 2 * length(covariatesbgan), 2)  # AIC formula: -2LL + 2k
BIC_val <- round(-2 * log_likelihoodgan + log(attr(model_igbgan, "nobs")) * length(covariatesbgan), 2)  # BIC formula

# Display the results
cat("Coefficients:\n")
print(resultsbgan)
cat("\nFrailty Variance (θ):", frailty_variancegan, "\n")
cat("Log-likelihood:", log_likelihoodgan, "\n")
cat("AIC:", AIC_val, "\n")
cat("BIC:", BIC_val, "\n")
